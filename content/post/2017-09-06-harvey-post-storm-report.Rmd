...
```{r tbl_wind, echo = FALSE}
kable(slp_df %>% 
        top_n(5L, Wind) %>% 
        select(ID, Station, Lat, Lon, Wind, WindDT) %>% 
        arrange(desc(Wind)), 
      caption = "Five highest wind observations (kts).")
```
