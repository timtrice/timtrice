...
```{r ref.label = "slp_normalize", echo = 3:4, eval = FALSE}
```
...
```{r ref.label = "slp_normalize", echo = c(9:10, 13, 16), eval = FALSE}
```
...
```{r ref.label = "slp_normalize", echo = 21:25, eval = FALSE}
```

Finally, I subset `slp_obs` and then with `slp_stations` make vector `slp`.

```{r ref.label = "slp_normalize", echo = c(28,31), eval = FALSE}
```
