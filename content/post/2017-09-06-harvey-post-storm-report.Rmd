...
```{r plot_al092017_gulf, echo = FALSE}
bp + 
...
```{r plot_al092017_wind, echo = FALSE}
AL092017 %>% 
...
```{r plot_al092017_pres, echo = FALSE}
AL092017 %>% 
...
```{r plot_al092017_wind_pres, echo = FALSE}
ggplot(data = AL092017, aes(x = Wind, y = Pressure)) + 
  geom_point() + 
  theme_bw() + 
...
```{r, echo = FALSE}
ggplot(data = filter(slp_df, !is.na(Wind), !is.na(Pres)), 
       aes(x = Wind, y = Pres, color = WindRmks)) + 
  geom_point() + 
  theme_bw() + 
  theme(legend.position = "bottom", 
        legend.direction = "horizontal") + 
  labs(title = "Surface Wind/Pressure Observations", 
       x = "Wind (kts)", 
       y = "Pressure (mb)")
```

```{r, echo = FALSE}
ggplot(data = filter(slp_df, !is.na(Gust), !is.na(Pres)), 
       aes(x = Gust, y = Pres, color = GustRmks)) + 
  geom_point() + 
  theme_bw() + 
  theme(legend.position = "bottom", 
        legend.direction = "horizontal") + 
  labs(title = "Surface Gust/Pressure Observations", 
       x = "Gust (kts)", 
       y = "Pressure (mb)")
```
...
(tmp_plot <- bp + 
  ...
  ggtitle("Sea Level Pressure - Hurricane Harvey"))
```
...
```{r tbl_pres, echo = FALSE, results = "asis"}
slp_df %>% 
  top_n(-5L, Pres) %>% 
  select(-c(Wind:GustRmks)) %>% 
  arrange(Pres) %>% 
  pandoc.table(caption = "Five lowest pressure observations (mb).", 
               style = "rmarkdown", justify = "left", split.table = Inf)
```
...
Not surprisingly, many of the locations that recorded the lower pressure values also recorded the higher wind values.
...
```{r tbl_wind, echo = FALSE, results = "asis"}
slp_df %>% 
  top_n(5L, Wind) %>% 
  select(ID, Station, Lat, Lon, Wind, WindDT) %>% 
  arrange(desc(Wind)) %>% 
  pandoc.table(caption = "Five highest wind observations (kts).", 
               style = "rmarkdown", justify = "left", split.table = Inf)
```
...
```{r tbl_wind_select, echo = FALSE, results = "asis"}
tmp <- slp_df %>% 
...
pandoc.table(tmp, caption = "Select Wind observations.", style = "rmarkdown", 
             justify = "left", split.table = Inf)
```

```{r plot_wind_select, echo = FALSE}
bp + 
  geom_point(data = tmp, 
             aes(x = Lon, y = Lat, color = Wind), shape = 9) + 
  ...
  scale_color_gradientn(colors = rev(terrain.colors(10))) + 
  coord_equal(xlim = c(-97.5, -96.75), ylim = c(27.5, 28.25)) + 
  ggtitle("Select Wind Observations")
```

### Rain

Without a doubt, the main story from Hurricane Harvey will be the rain; some areas in southeast Texas recording over 50 inches. 
...
```{r tbl_rain, echo = FALSE, results = "asis"}
rain_df %>% 
  top_n(5L, Rain) %>% 
  arrange(desc(Rain)) %>% 
  select(-RainRmks) %>% 
  pandoc.table(caption = "Top 5 Rainfall Amounts", 
               style = "rmarkdown", justify = "left", split.table = Inf)
```
...
```{r tbl_tors, echo = FALSE, results = "asis"}
tor_df %>% 
  pandoc.table(caption = "Tornado Remarks, Hurricane Harvey", 
               style = "rmarkdown", justify = "left", split.table = Inf)
```
...
pandoc.table(efs, caption = "Enhanced Fujita Scale", style = "rmarkdown", 
             justify = "left")
```
