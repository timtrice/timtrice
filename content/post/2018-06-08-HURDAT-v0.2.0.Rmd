...
AL %>% 
  ...
  geom_ribbon(alpha = 0.35, color = NA, aes(ymin = 0, ymax = Storms)) + 
  geom_line(size = 0.25) + 
  scale_x_continuous(limits = c(1851, 2017), 
                     breaks = NULL, 
                     minor_breaks = seq(1850, 2020, by = 20), 
                     expand = c(0, 0)) + 
  ...
  labs(title = "Atlantic Basin", 
       subtitle = "Tropical Cyclone Activity by Decade", 
       caption = sprintf("@timtrice, HURDAT v%s", packageVersion("HURDAT")), 
       x = element_blank(), 
       y = element_blank())
```
