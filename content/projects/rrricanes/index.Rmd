...
To go along with my [HURDAT](http://timtrice.net/projects/`HURDAT`/) package I'm releasing [rrricanes](https://github.com/timtrice/rrricanes), version 0.1.0. This library is *not* in cran and will not be submitted until release 0.2.0.
...
`rrricanes` access and parses real-time data and returns it in dataframe format. Unlike `HURDAT`,`rrricanes` can get the the latest data available for any cyclone in the Atlanic or northeastern Pacific ocean. 

Keep in mind `HURDAT` is a re-analysis dataset and is meant as a compliment to `rrricanes`. `rrricanes`, at this time, can only access it's set of data back to 1998. But, the data is much more comprehensive than `HURDAT` which goes back to 1851.

Additionally, some data in `rrricanes` may not match up with that for `HURDAT`. For example, you may find an observation for a specific storm at a specific time has different values for latitude. This is because originally the National Hurricane Center (NHC) believed the center of the hurricane to be in one location. But, after re-analysis, determined it was somewhere else.

## Installing `rrricanes`

With all of that said, let's examine `rrricanes`. First, install the library using `devtools`.
...
`rrricanes` should work with any R version >= 3.2.3.
...
I've tried to be as detailed as possible while keeping the vignette short. Here, I want to focus on what I think the best aspect of `rrricanes` is so far. 
...
* gis data: I do not know how far back this data goes but it is readily avaiable for past and current storms. There is an [issue](https://github.com/timtrice/rrricanes/issues/47) opened for this to be added; but, no milestone as of yet. 
...
2. [Submit an issue](https://github.com/timtrice/rrricanes/issues). Errors in data will be given top priority and hotfixes will be created to address any corrections that can be made. 
...
Thank you for taking the time to check out `rrricanes`. Please feel free to check out [HURDAT](http://timtrice.net/projects/hurdat/) as well. 

<small>`Hurricanes` was renamed to `rrricanes`. This page is updated to reflect this change.</small>
